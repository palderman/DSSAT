...
test_cols_check <- function(data,
  ...
  for(nm in names(data)){
    if(nm %in% missing){
      ...
      if(!is.null(list_cols) && nm %in% list_cols){
        ...
        expect_equal(data[[nm]], rep(na_val, nrow(data)), label = nm)
      }
    }
  }
  ...
  if(!is.null(expected_vals)){
    for(nm in names(expected_vals)){
      expect_equal(data[[nm]], expected_vals[[nm]], label = nm)
    }
  }
  ...
  if(!is.null(list_cols)){
    for(nm in list_cols){
      expect_true(is.list(data[[nm]]), label = nm)
      if(!is.null(list_col_length)){
        ...
        for(i in 1:length(data[[nm]])){
          expect_equal(length(unlist(data[[nm]][i])), list_col_length[i], label = nm)
        }
      }
      if(!is.null(list_col_groups)){
        for(i in 1:nrow(data)){
          for(g in 1:length(list_col_groups)){
            ...
            expect_equal(length_range[1], length_range[2], label = lbl)
          }
        }
      }
    }
